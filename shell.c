...
int runCommand(struct job newJob, struct jobSet * jobList, 
	...
	// in every process and stoppedProgs = 0 in the job

        // Syntax check

        // check if argument exists
        if(newJob.progs[0].argv[1] == NULL){
            printf("syntax error, no argument found\n");
            return 1;
        }

        // check if fisrt char is '%' which translated to 37 in ASCI
        if(newJob.progs[0].argv[1][0] != 37){
            printf("syntax error, argument must start with %c\n", 37);
            return 1;
        }
        ...
        int i = 1;
        while (newJob.progs[0].argv[1][i] != '\0')
        {
            if(newJob.progs[0].argv[1][i] > 57 || newJob.progs[0].argv[1][i] < 48){
                printf("syntax error, after %c, argument must contain a number \n", 37);
                return 1;
            }
            i++;
        }
